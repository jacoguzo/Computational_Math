...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 21:54:30 2023
...
def stabilized(f, y0, t0, tf, h):
    ...
    for i in range(n):
        ...
        y[i + 1] = (y[i] + 10*h*np.sin(y[i])+ 10*h*y[i])/(1+10*h)
...
# Solve the ODE
t, y, y_err = stabilized(f, y0, t0, tf, h)
print(y)
print(t)
print(y_err)
...
plt.plot(t, np.cos(y), label="cos(y)", dashes=(3,2), color="green", lw=3)
...
plt.plot(t, y, label="Numerical solution:\n Stabilized Euler w/ h=0.22", dashes=(3,2), color="blue", lw=3)
#plt.plot(x_eu, y_eu, label="Numerical solution:\nEuler", dashes=(3,2), color="green", lw=3)

plt.legend(loc="best", fontsize=12)
plt.title(r"Solution to ODE: $y'(t) = 10sin(y(t))$ using Stabilized Euler with h =0.22")
plt.xlabel("t", fontsize=12)
plt.ylabel("y", fontsize=12)
plt.show()
